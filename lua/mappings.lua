...
utils.lnmap(
    ...
    ":q!<cr>",
    {
        desc = "close current bufferall without saving"
    }
)
vim.keymap.set(
    "n",
...
utils.nmap(
    ...
    "<c-u>zz",
    {
        desc = "center screen after page up"
    }
)
utils.nmap(
    "<c-d>",
    "<c-d>zz",
    {
        desc = "center screen after page down"
    }
)
vim.keymap.set(
    {"n", "v"},
    ...
    [[10j<cr>]],
    {
        desc = "moves over virtual (wrapped) lines down"
    }
)
vim.keymap.set(
    {"n", "v"},
    ...
    [[10k<cr>]],
    {
        desc = "moves over virtual (wrapped) lines up"
    }
)
vim.api.nvim_set_keymap(
    "n",
    ...
    "v:count == 0 ? 'gk' : 'k'",
    {
        ...
        expr = true,
        silent = true,
        desc = "moves up over virtual (wrapped) lines"
    }
)
vim.api.nvim_set_keymap(
    "n",
...
utils.nmap(
    ...
    "^",
    {
        desc = "move to first non-bkgtgtgtgtlank character of the line"
    }
)
utils.vmap(
    "<S-PageDown>",
...
vim.keymap.set(
    ...
    "mzJ`z",
    {
        desc = "join lines without spaces"
    }
)
vim.keymap.set(
    "n",
...
utils.nmap(
    ...
    "*``cgn",
    {
        desc = "replace word under cursor simultaneously"
    }
)
utils.nmap(
    "<leader>X",
...
utils.xmap(
    ...
    ":'<,'>:normal @q<CR>",
    {
        desc = "run macro from q register on visual selection"
    }
)
utils.tmap(
    "<ESC>",
...
vim.keymap.set(
    ...
    "^I-<Space>[<Space>]<Space><Esc>^j",
    {
        remap = true,
        silent = false,
        desc = "prepend markdown list item on line"
    }
)
utils.vmap(
    "srt",
...
utils.nmap(
    ...
    "i<space><esc>",
    {
        desc = "Insert space in normal mode"
    }
)
utils.nmap(
    "<leader>sq",
...
utils.lnmap(
    ...
    '"*yy',
    {
        desc = "yank line to the clipboard buffer"
    }
)
utils.nmap(
    "<leader>df",
...
utils.nmap(
    ...
    ":%y<cr>",
    {
        desc = "yank file under cusror to the clipboard buffer"
    }
)
utils.nmap(
    "<leader>yw",
    '"+yiw',
    {
        desc = "yank word under cusror to the clipboard buffer"
    }
)
utils.nmap(
    "<leader>yW",
...
utils.lnmap(
    ...
    '"*P',
    {
        desc = "paste from clipboard buffer before the cursor"
    }
)
utils.nmap(
    "<leader>0",
...
utils.nmap(
    ...
    '"1p',
    {
        desc = "paste from 1 (latest delete)"
    }
)
utils.nmap(
    "<leader>2",
...
utils.nmap(
    ...
    ":pu<CR>",
    {
        desc = "paste from clipboard buffer after the cursor"
    }
)
utils.lnmap(
    "dl",
    '"_dd',
    {
        desc = "delete line to black hole register"
    }
)
utils.lnmap(
    "d_",
    '"_D',
    {
        desc = "delete till end of line to black hole register"
    }
)
utils.xmap(
    "<leader>d",
...
utils.imap(
    ...
    "<c-o>daw",
    {
        desc = "delete word forward in insert mode"
    }
)
vim.keymap.set(
    "i",
...
utils.nmap(
    ...
    ":.w !bash -e <cr>",
    {
        desc = "execute current line and output to command line"
    }
)
utils.nmap(
    "<leader>eX",
    ":%w !bash -e <cr>",
    {
        desc = "exexute all lines and output to command line"
    }
)
utils.nmap(
    "<leader>el",
    ...
    {
        silent = false,
        desc = "execute current line and replace with result"
    }
)
utils.nmap(
    "<leader>eL",
    ":% !bash % <cr>",
    {
        desc = "execute all lines and replace with result"
    }
)
utils.lnmap(
    "cx",
...
) -- git browse current file and selected line in browser
...
utils.imap("<expr>", "<Tab> luasnip#expand_or_jumpable() ? '<Plug>luasnip-expand-or-jump' : '<Tab>") -- -1 for jumping backwards.
-- For changing choices in choiceNodes (not strictly necessary for a basic setup).
utils.imap("<expr>", "<C-E> luasnip#choice_active() ? '<Plug>luasnip-next-choice' : '<C-E>")
utils.imap("<expr>", "<C-E> luasnip#choice_active() ? '<Plug>luasnip-next-choice' : '<C-E>")
